...
- FR11: System can check extension availability on any VCS repository declared in the installation's composer.json, using Composer CLI or git CLI as fallback, regardless of hosting provider
- ~~FR12: System can check extension availability on GitLab repositories, including private instances with authentication~~ *(removed — merged into FR11)*
- ~~FR13: System can check extension availability on Bitbucket repositories (tags, branches, version constraints)~~ *(removed — merged into FR11)*
- FR14: System can aggregate availability data across all sources into a unified availability status per extension
- FR15: System can detect abandoned or unmaintained extensions based on latest tag/commit age from Composer metadata or git
...
- NFR5: If an external API (TER, Packagist, any git repository) is unavailable, the analysis completes with partial results and clear indication of what data is missing
...
- NFR11: Private VCS hosts are accessed via Composer `auth.json` or SSH agent — the tool does not manage SSH keys or tool-specific tokens
...
- FR9–FR10 TER/Packagist: COMPLETE
- FR11 VCS availability (GitHub-only): PARTIALLY COMPLETE (existing GitHubClient works; replacing with Composer-based approach per Sprint Change Proposal 2026-03-26)
...
- FR11: Epic 2 (partially complete) — Check extension availability on any VCS source declared in composer.json (consolidates former FR11/FR12/FR13; existing GitHub-only implementation being replaced with Composer-based approach)
- ~~FR12: Epic 2 — Check extension availability on GitLab~~ *(removed — merged into FR11)*
- ~~FR13: Epic 2 — Check extension availability on Bitbucket~~ *(removed — merged into FR11)*
- FR14: Epic 2 — Aggregate availability data across all sources (completing coverage)
- FR15: Epic 2 — Detect abandoned/unmaintained extensions via latest tag/commit age
...
Developer gets version availability data for extensions hosted on any VCS provider (GitHub, GitLab, Bitbucket, Codeberg, self-hosted, etc.), using Composer CLI as primary resolver and generic git CLI as fallback. Replaces per-provider API clients with provider-agnostic resolution. Transition: existing `GitHubClient` remains wired until Story 2.5; removed in Story 2.6.

### Story 2.0: Research Spike — Composer CLI VCS Resolution Capabilities

As a developer,
I want a timeboxed (1 day) research spike validating Composer CLI behavior for VCS resolution,
So that Stories 2.2 and 2.3 are built on verified assumptions rather than documentation alone.

**Acceptance Criteria:**

- **Given** a test installation with extensions from GitHub, GitLab, Bitbucket, and a self-hosted git URL
- **When** the following Composer CLI commands are tested against it:
  - `composer show -o -d /path --format=json` (batch outdated)
...
- **Then** the spike documents: which command variant gives the needed data (available versions, compatibility), whether `--working-dir` passes auth through from the target installation's `auth.json`, performance characteristics (batch vs per-package), and any edge cases
- **And** `git ls-remote --tags <url>` and `git ls-remote --heads <url>` are tested as Tier 2 fallback, documenting output format and auth behavior (SSH agent)
- **And** findings are written to `documentation/implementation/development/feature/VcsResolutionSpike.md`
- **And** the document recommends a specific Composer command variant for Story 2.2 and confirms or rejects `git ls-remote` as viable fallback for Story 2.3
- **And** no production code is written — this is research only

**Note:** This story is a prerequisite for Stories 2.2 and 2.3. Method signatures and exact CLI invocations in those stories depend on spike findings.

---

### Story 2.1: Composer Source Parser (Simplified)

As a developer,
I want the tool to extract VCS URLs from the analyzed installation's `composer.lock` and `composer.json`,
So that downstream VCS resolution has a reliable list of repository URLs without provider-type classification.

**Acceptance Criteria:**

- **Given** a TYPO3 installation with a `composer.lock` containing packages with `source.url` entries and a `composer.json` with a `repositories` section
- **When** `ComposerSourceParser::parse(string $installationPath)` is called
- **Then** it returns a `DeclaredRepository[]` array where each entry carries `url` only (no `type` field, no provider classification)
- **And** primary source is `composer.lock` `packages[].source.url`; fallback is `composer.json` `repositories[].url`
- **And** if both files are absent, an empty array is returned without error
- **And** malformed JSON is handled gracefully with a logged warning and empty result — never a crash
- **And** `ComposerSourceParser` lives in `Infrastructure/Discovery/` and has no domain-layer imports
- **And** `DeclaredRepository` is a readonly value object with `url` property only, in `Infrastructure/ExternalTool/`
...
- **Given** Story 2.5 is complete and all tests pass with the new VCS resolution chain
- **When** cleanup is performed
- **Then** the following files are deleted: `GitProviderInterface.php`, `AbstractGitProvider.php`, `GitHubClient.php`, `GitProviderFactory.php`, `GitProviderException.php` (evaluate — may keep as `VcsResolutionException`), `GitRepositoryAnalyzer.php`, `RepositoryUrlHandler.php` (evaluate — may still be needed for URL normalization), `GitRepositoryHealth.php`
- **And** all associated test files for the deleted classes are removed
- **And** `services.yaml` entries for deleted classes are removed
- **And** no remaining source file imports or references deleted classes
- **And** all tests pass after cleanup
- **And** PHPStan Level 8 reports zero errors
