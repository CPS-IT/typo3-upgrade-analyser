...
final readonly class InstallationDiscoveryResult implements SerializableInterface
{
    ...
    public function getValidationIssuesBySeverity(ValidationSeverity $severity): array
    {
        return array_filter(
            ...
            static fn (ValidationIssue $issue): bool => $issue->getSeverity() === $severity,
        );
    }
    ...
    public function hasBlockingIssues(): bool
    {
        return !empty(array_filter(
            $this->validationIssues,
            static fn (ValidationIssue $issue): bool => $issue->isBlockingAnalysis(),
        ));
    }
    ...
    public function getSummary(): string
    {
        ...
        if ($this->hasValidationIssues()) {
            ...
            $blockingCount = \count(array_filter($this->validationIssues, static fn ($issue): bool => $issue->isBlockingAnalysis()));
    ...
    public function getStatistics(): array
    {
        $stats = [
            ...
            'supported_strategies' => \count(array_filter($this->attemptedStrategies, static fn (array $attempt): mixed => $attempt['supported'] ?? false)),
            'validation_issues' => \count($this->validationIssues),
            'blocking_issues' => \count(array_filter($this->validationIssues, static fn ($issue): bool => $issue->isBlockingAnalysis())),
        ];
    ...
    public function toArray(): array
    {
        return [
            ...
            'validation_issues' => array_map(static fn (ValidationIssue $issue): array => $issue->toArray(), $this->validationIssues),
            'validation_summary' => [
