...
class Extension implements \JsonSerializable
{
    ...
    {
        return array_filter($this->files, static fn ($file): bool => str_ends_with($file, '.php'));
    }

    public function getTcaFiles(): array
    {
        return array_filter($this->files, static fn ($file): bool => str_contains($file, '/TCA/') && str_ends_with($file, '.php'));
    }

    public function getTemplateFiles(): array
    {
        return array_filter($this->files, static fn ($file): bool => str_ends_with($file, '.html') || str_ends_with($file, '.fluid'));
    }
