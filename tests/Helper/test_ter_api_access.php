            ...
            if (is_array($result['data'])) {
                ...
                if (isset($result['data']['versions']) && is_array($result['data']['versions'])) {
                    ...
                    foreach ($recentVersions as $version => $versionData) {
                        if (isset($versionData['typo3_versions']) && is_array($versionData['typo3_versions'])) {
                            foreach ($versionData['typo3_versions'] as $typo3Version) {
                                if (!in_array($typo3Version, $typo3Compatibility)) {
                    ...
                    if (!empty($typo3Compatibility)) {
                        ...
                        if (in_array(12, $typo3Compatibility)) {
                            echo "  ✓ TYPO3 12.x support: YES\n";
                        } else {
                            echo "  ✗ TYPO3 12.x support: NO\n";
                        }
                        
                        if (in_array(11, $typo3Compatibility)) {
...
$newsTest = array_filter($results, fn($r) => str_contains($r['url'], '/extension/news') && !str_contains($r['url'], '/versions'));
if (!empty($newsTest)) {
...
$realurlTest = array_filter($results, fn($r) => str_contains($r['url'], '/extension/realurl') && !str_contains($r['url'], '/versions'));
